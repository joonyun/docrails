...
[Eager Loading Associations] 관계 선적재(先積載: Eager Loading)
--------------------------

선적재라 함은 쿼리 회수를 가능한한 줄여, `Model.find` 메소드가 반환하는 객체들과 연관되는 레코드를 미리 객체로 로딩하는 기전을 말한다.

**N + 1 쿼리 문제**

아래의 코드를 가정해 보기로 한다. 여기서는 10명의 고객을 불러와 각자의 우편번호를 출력한다.
...
이 코드는 일견 문제가 없어 보인다. 그러나 문제는 실행되는 쿼리의 총수에 있다. 위의 코드는 총 11번의 쿼리가 수행되는데, 10명의 client 정보를 불러오기 위해 1번의 쿼리를 수행하고, 각 client의 address를 로드하기 위해서 한번씩의 쿼리를 더 수행해야 하므로 10번의 쿼리가 추가로 수행된다. 

**N + 1 쿼리 문제 해결법**

액티브레코드를 이용하면 로드되어야할 모든 관계를 미리 명시할 수 있게 해준다. `Model.find` 를 호출하는 `includes` 메소드를 명시함으로써 가능하게 된다. `includes`를 사용하면 액티브레코드가 최소한의 쿼리를 수행하여 명시된 모든 관계를 로드하도록 보장한다. 

위의 코드는 address를 미리 로드하기 위해 `Client.limit(10)`를 아래와 같이 수정할 수 있다.
...
이 코드는 이전 코드에서 **11**번의 쿼리를 수행한 것과는 상반되게, 단 **2**번의 쿼리만을 수행하게 해 준다. 
...
### [Eager Loading Multiple Associations] 다중관계 선적재

액티브레코드는 `includes` 메소드에 배열, 해쉬, 도는 배열/해쉬로 구성된 중첩 해쉬를 넘겨 주어 한번의 `Model.find` 호출시에 여러개의 관계를 미리 로딩할 수 있게 해준다.


#### [Array of Multiple Associations] 다중관계 배열형

```ruby
Post.includes(:category, :comments)
```

이 코드는 모든 post와 각 post와 연관된 category와 comments를 미리 로딩한다.

#### [Nested Associations Hash] 중첩관계 해쉬형

```ruby
Category.includes(posts: [{comments: :guest}, :tags]).find(1)
```
이 코드는 1번 id를 가지는 category를 불러와 이와 연관된 posts, 해당 post의 tags와 comments, 그리고 각 comment의 guest 관계를 모두 선적재하게 될 것이다.

### [Specifying Conditions on Eager Loaded Associations] 선적재된 관계에 조건 명시하기

액티브레코드는, `joins`와 같이, 선적재된 관계에 대해서 조건을 명시할 수 있게 해주지만, 추천되는 방법은 [joins](#joining-tables)를 대신 사용하는 것이다.

그러나, 반드시 관계를 선적재해야 한다면 `where` 메소드를 사용할 수 있다.

```ruby
Post.includes(:comments).where("comments.visible" => true)
```

이 코드는, `joins` 메소드가 `INNER JOIN` 함수를 사용하여 쿼리문을 작성하는 것과는 달리, `LEFT OUTER JOIN` 함수를 포함하는 쿼리문을 생성하게 될 것이다.

```ruby
  SELECT "posts"."id" AS t0_r0, ... "comments"."updated_at" AS t1_r5 FROM "posts" LEFT OUTER JOIN "comments" ON "comments"."post_id" = "posts"."id" WHERE (comments.visible = 1)
```

여기서 `where` 조건이 없다면 두번의 쿼리문이 생성될 것이다.

`includes` 쿼리를 사용할 때, 특정 post에 연관된 comments가 없다하더라도, 모든 posts가 로드될 것이다. INNER JOIN인 `joins`를 사용할 경우에는 join 조건이 **반드시** 충족되어야 하고 그렇지 못할 경우에는 반환되는 레코드가 없게 될 것이다.
