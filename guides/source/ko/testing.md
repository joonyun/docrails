...
에러가 발생할 때 어떻게 되는지 알아 보기 위해 아래와 같이 에러를 포함하는 테스트 하나를 작성합니다. 
...
이제 테스트를 실행하면 콘솔에서 더 많은 결과를 볼 수 있게 됩니다. 
...
결과에서의 `E` 문자는 테스트 상의 에러를 표시해 줍니다. 

NOTE: 각 테스트 메소드는 에러가 발생하거나 가정이 실패하는 즉시 바로 중단되지만, 테스트 셋은 다음 테스트 메소드와 함께 계속 진행됩니다. 또한, 모든 테스트 메소드는 알파벳순으로 실행됩니다. 

### [What to Include in Your Unit Tests]단위 테스트에서 포함해야 할 사항

문제가 있을 수 있는 모든 것에 대해서 테스트를 작성하는 것이 이상적입니다. 유효성 검증 각각에 대해서 적어도 하나의 테스트를 작성하고, 모델의 모든 메소드에 대해서도 최소한 하나의 테스트를 작성하는 것이 좋습니다. 

### [Assertions Available]사용 가능한 가정들 

지금까지 사용 가능한 가정들 중 몇가지를 맛보기로 알아 보았습니다. 가정이란 마치 테스트를 하는 일벌과 같습니다. 가정이란, 구현한 내용들이 계획한 대로 제대로 동작하는지를 확신하기 위해, 실제로 확인 작업을 하는 것입니다.
