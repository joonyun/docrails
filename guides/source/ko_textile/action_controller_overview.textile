...
class ClientsController < ActionController::Base
  ...
  # 이 액션은 POST 데이터 파라메터를 이용합니다. 이 매개변수는 거의 대부분 사용자가
...
주목할 것은 +params+ 해쉬는 실제로 Active Support 모듈의 +HashWithIndifferentAccess+ 클래스의 인스턴스이며, 키를 심볼이나 문자열 형태로 사용할 수 있게해 주는 해쉬와 같이 동작한다는 것입니다.
...
또한, 레일스의 initializer 에 +config.wrap_parameters+ 를 "on" 상태로 설정하거나 컨트롤러 내에서 +wrap_parameters+ 메소스를 호출하게 되면 JSON/XML 매개변수에서 root 엘리먼트를 생략해도 문제가 없을 것입니다. 이렇게 되면 디폴트로 컨트롤러의 이름에 해당하는 키 이름으로 매개변수들이 복제되어 할당될 것입니다. 따라서 위의 매개변수는 다음과 같이 작성될 수 있습니다:
...
"API documentation":http://api.rubyonrails.org/classes/ActionController/ParamsWrapper.html 를 참조하게 되면 매개변수를 할당받아 오게 될 키 이름이나 특수한 형태의 매개변수를 변경할 수 있습니다.
